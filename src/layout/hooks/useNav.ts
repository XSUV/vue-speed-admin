...
import { useGlobal, isAllEmpty } from '@/utils/tools';
...
export function useNav() {
  ...
  const speedApp = useAppStoreHook();
  ...
  const isCollapse = computed(() => {
    return !speedApp.getSidebarStatus;
  });

  const device = computed(() => {
    return speedApp.getDevice;
  });
